...
const Formpage1 = () => {
  ...
  return (
    <div className='w-full h-full flex flex-col justify-center items-center'>
      ...
      <div className="w-[852px] h-[584.17px] rounded-2xl border border-white backdrop-blur-[22px] mt-[5vh] justify-center flex flex-col items-center max-sm:border-none max-sm:w-screen max-sm:backdrop-blur-0 max-sm:mt-[-7rem]">
        <input className="w-[754.71px] h-[70.73px] bg-zinc-300 bg-opacity-0 rounded-[15px] border border-zinc-300 placeholder:text-neutral-500 text-[25px] font-normal font-['Montserrat'] indent-[3rem] max-sm:w-[280px] max-sm:h-[50px] max-sm:bg-zinc-900 max-sm:text-[15px] max-sm:indent-[2rem]" value={name} onChange={(e)=>setName(e.target.value)} placeholder='Name *' type='text' required></input>
      ...
      <button className="w-[189px] h-[67px] bg-violet-600 rounded-[10px] border-white text-white text-3xl font-bold font-['Poppins'] mt-[7vh] mb-[10vh] max-sm:w-[280px] max-sm:h-[40px] max-sm:mt-[-7rem] max-sm:text-[22px]" onClick={handelSubmit}>Next</button>
    </div>
  );
}
